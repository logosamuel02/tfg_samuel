...
def heatmap_messages(data):
    ...
    fig = px.imshow(data_cross.to_numpy(), x=agents, y=agents, color_continuous_scale="Viridis", aspect="auto")
...
def heatmap_sizes(data):
    ...
    data.sender = list(map(lambda x: x.split("@")[0], data.sender))
    data.to = list(map(lambda x: x.split("@")[0], data.to))
    data_cross = data.groupby(["sender", "to"])["size"].sum().div(1024*1024).round(0).unstack().fillna(0)
    agents = data.sender.unique()
    fig = px.imshow(data_cross.to_numpy(), x=agents, y=agents, color_continuous_scale="Viridis", aspect="auto")
    fig.update_traces(text=data_cross, texttemplate="<b>%{text}</b>")
    fig.update_xaxes(side="top")
...
def create_distribution_data(data):
    ...
    data["timestamp_minutes"] = data.timestamp.apply(lambda x: int((x - min_date).total_seconds() / 60.0))
    return data
...
def distribution_info(data):
    fig = px.histogram(data, x="timestamp_minutes", y="size", color_discrete_sequence=['indianred'])
    fig.update_layout(title_text="Distribución de mensajes en el tiempo")
    return fig

def distribution_info_type(data):
    fig = px.histogram(data, x="timestamp_minutes", y="size", color="type")
    fig.update_layout(title_text="Distribución de mensajes en el tiempo por tipo de mensaje")
    return fig
